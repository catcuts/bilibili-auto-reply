...
const createProxyClient = async () => {
    ...
    const baseURL = isBrowser ? '/api/proxy' : 'http://localhost:3000/api/proxy';
    ...
    getUserInfo: async cookies => {
        try {
            ...
            if (useProxy) {
                ...
                const response = await biliApi.get('https://api.bilibili.com/x/web-interface/nav', {
                    headers: {
                        Cookie: cookies,
                    },
                });
