...
const RuleForm: React.FC<RuleFormProps> = ({ initialData, userId, onSubmit, onCancel }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-lg shadow">
      ...
      <div>
        <label htmlFor="responseTemplate" className="block text-sm font-medium text-gray-700">
          回复模板
        </label>
        <textarea
          id="responseTemplate"
        ...
        {errors.responseTemplate && <p className="mt-1 text-sm text-red-600">{errors.responseTemplate}</p>}
      </div>
